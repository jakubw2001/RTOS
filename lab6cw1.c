...
int main ()
{
	...
	struct sched_param param[NUM_THREADS];	/* struktura opisujaca parametry szeregowania */
    int priorities[] = {1, 2, 3, 4};
    int policies[] = {SCHED_FIFO, SCHED_RR, SCHED_RR, SCHED_FIFO};
	void* status;

	for(i = 0; i < NUM_THREADS; i++)
	{
	    sched_getparam(0, &param[i]);
        pthread_attr_init(&attr[i]);
        pthread_attr_setschedpolicy(&attr[i], policies[i]);
		...
		rc = pthread_create (&thread_id[i], &attr[i], &start_routine, (void *)i);
